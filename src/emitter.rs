...
pub struct Emitter<'a, C: CodeMap> {
    dst: Destination<'a>,
    cm: Option<&'a C>,
}

struct FileWithAnnotatedLines<F: File> {
    file: Arc<F>,
    lines: Vec<Line>,
    multiline_depth: usize,
}

impl<'a, C: CodeMap> Emitter<'a, C> {
    /// Creates an emitter wrapping stderr.
    pub fn stderr(color_config: ColorConfig, code_map: Option<&'a C>) -> Self {
   ...
   pub fn new(dst: Box<Write + Send + 'a>, code_map: Option<&'a C>) -> Self {
       Emitter {
           dst: Raw(dst),
           cm: code_map,
       }
   ...
   fn preprocess_annotations(cm: Option<&'a C>, spans: &[SpanLabel<C>]) -> Vec<FileWithAnnotatedLines<C::File>> {
       fn add_annotation_to_file<'a, F: File>(file_vec: &mut Vec<FileWithAnnotatedLines<F>>,
                                 file: Arc<F>,
                                 line_index: usize,
                                 ann: Annotation) {
   ...
   fn get_max_line_num(&mut self, diagnostics: &[Diagnostic<C>]) -> usize {
   ...
   pub fn emit(&mut self, msgs: &[Diagnostic<C>]) {
